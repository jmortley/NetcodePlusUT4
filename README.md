...
- **View-bob / weapon-bob sync (client)** — `Game.ini` (`EyeOffsetGlobalScaling` / `WeaponBobGlobalScaling`, i.e. the Settings → Player sliders) is enforced as the single source of truth. Stock re-stamps every new PlayerController from the cloud profile's `ViewBob`/`WeaponBob`, which can silently hold the 1.0 default while the slider reads 0 — full view bob (and a moved hitscan origin: `EyeOffset` is part of `GetPawnViewLocation`) with no UI indication. The plugin re-applies the ini value to the live controller and the profile at ~4 Hz and persists the profile; `[BobSync]` lines in the client log show any correction. Keys missing from `Game.ini` are left alone.
