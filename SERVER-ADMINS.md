...
| `ncp.RenderCredit` | `1` | The render gate's inverse, for fire that can never claim (link beam ticks): a shot that misses the raw rewind capsule is re-tested against the target's **render-time** capsule and credited when the shooter's screen genuinely showed the hit. Direction/timing are fully server-derived — this is not client hit detection. `0` restores raw-rewind-only (pre-hotfix-4) beams. `[RenderCredit]` lines appear under `Log LogUTWeaponFix Verbose`. |
| `ncp.RenderCreditSlack` | `0` | Extra radius (uu) forgiven by the render-credit re-test. Deliberately 0 — this is the acceptance direction; widen only with log evidence. |
