...
| `ncp.ClientFireOrigin` | `1` | **Click-anchored hitscan origin.** The fire RPC now carries the client's own fire-start location; when it validates against the shooter's click-frame `SavedPositions` entry (XY within `ncp.ClientFireOriginToleranceXY`, Z inside the capsule column) the server traces from **that** point instead of its re-simulated pawn. Removes the moving-shooter origin drift (uncorrected 14 uu prediction band, server-side view-bob integration at scale 1.0, +40/+80 ms resends firing from a later position). Hitscan only; projectiles keep the parallax path. `0` = stock server-pawn origin. Needs matching client and server builds (RPC signature change). |
| `ncp.ClientFireOriginToleranceXY` | `28` | Max horizontal distance (uu) between the client's reported origin and the server's click-frame pawn position before the origin claim is rejected (falls back to the server origin, logged under `ncp.FireAnchorDebug`). 18 uu = the RC `MaxPositionErrorSquared` (324) band + 1 uu `ClientLoc` quantization + slack. This is also the lateral cheat window — keep it well under the 40 uu claim padding. |
| `ncp.FireAnchorRewind` | `1` | Adds the fire RPC's **lateness** (server now − time the shooter's click-frame move was processed) to the hitscan target rewind, so a resent/retransmitted/head-of-line-blocked shot samples targets at the click epoch instead of `now − ½RTT`. Zero on the normal path (fire lands in the packet right behind its move), so healthy connections are unchanged. Still capped by the weapon's `MaxRewindMs`. `0` = ping-average rewind only. |
| `ncp.FireAnchorDebug` | `0` | Per-shot `[FireAnchor]` log line: whether the click move was found (`anchor=move` / `nearest`), lateness, origin delta vs the server pawn, and the verdict (`origin=client` / `REJECT->server`). Diagnostic only. A steady stream of `anchor=nearest` on healthy connections means the click move was not flagged `bShotSpawned` for that dispatch path — report it. |
